...
from utils import get_freq, evaluate_attack, get_correct, get_loader, load_model, load_data
...
def correct_sample(model, args):
    
    loader = get_loader(args, 1)

    pbar = tqdm.tqdm(loader, unit='batches', leave=False, total=len(loader))
    correct, total = 0.,0.
...
def main():
    ...
    train_loader, valid_loader, test_loader = load_data(args.root, args.batch_size)
    ...
    acc, corr, total = correct_sample(model, args)
